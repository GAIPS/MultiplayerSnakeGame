...
from msg import Agent
from msg import Action, PRE_IDS
...
class AStarAgent(Agent):
    ...
    def astar(self, observation, _goal, pos=None, direction_ptr=None):
        ...
        cost_so_far[start] = 0
        if (direction_ptr is not None):
            direction_ptrs[start] = direction_ptr
        else:
            direction_ptrs[start] = \
                observation["direction_ptr"][observation["self"]]
        ...
        while frontier:
            ...
            for next_node in utils.get_neighbours(direction_ptrs[current],
                                                  observation,
                                                  current):
                # print(next_node)
                ...
                if (next_node not in cost_so_far or
                    ...
                    direction_ptrs[next_node] = utils.get_direction_ptr(
                            observation, current, next_node)
...
class AStarNearest(AStarAgent):
    ...
    def action(self, observation) -> int:
        ...
        if (self.reevaluateEvery is None):
        ...
        return utils.get_action(
                observation["direction_ptr"][observation["self"]],
                observation, head, next_pos)
